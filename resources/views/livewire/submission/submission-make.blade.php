<x-form title="{{ !auth()->user()->hasRole(5) ? 'Submissões' : 'Minhas Submissões' }}" wire:submit.prevent="store"
        ...
        <div class="flex flex-col-reverse xl:flex-row">
            <div class="mt-6 flex-1 xl:mt-0">
                <x-form.row cols="2">
                    <x-form.field label="Evento *">
                        <x-input.select :options="$form->events" wire:model.defer="form.event_id"
                            selectedOption="{{ old('form.event_id', $form->event_id) }}" />
                        <x-input.error for="form.event_id" class="mt-2" />
                    </x-form.field>
                    <x-form.field label="Eixo *">
                        <x-input.select :options="$form->axles" wire:model="form.axle_id"
                            selectedOption="{{ old('form.axle_id') ?? '' }}" />
                ...
                <x-form.row cols="2">
                    ...
                    @else
                        <x-form.field label="Arquivo PDF">
                            <input type="file" wire:model="form.file" name="form.file" accept=".pdf">
                        ...
                        @if ($form->object->file_new)
                            <x-form.field label="Arquivo PDF com Alterações">
                                <a href="{{ Storage::url($form->object->file_new) }}" target="_blank"
                                    class="text-blue-500 ml-2 w-40">
                                    <i class="fa-solid fa-external-link fa-lg"></i>
                                </a>
                            </x-form.field>
                        @else
                            <x-form.field label="Arquivo PDF com Alterações">
                                <input type="file" wire:model="form.file_new" name="form.file_new" accept=".pdf">
                                <x-input.error for="form.file_new" class="mt-2" />
                            </x-form.field>
                        @endif
                    ...
                    <x-form.row cols="2">
                        ...
                        <x-form.field label="Status">
                            <x-input.select :options="$form->statusOptions" placeholder wire:model="form.status"
                                selectedOption="old('form.status') ?? ''" disabled/>
    ...
    @endif
    <x-slot name="actions">
        <x-link.default href="{{ route('dashboard/submissions') }}">Cancelar</x-link.default>
